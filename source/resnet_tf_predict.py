...
print ('creating validation features with single best model')
...
model.load_weights("weights/resnet_augv2_17class_4a_pseudo_plus_validset_2epochs.h5")
...
def make_staged_predictions_v2(model,num_parts, nb_aug=1,output_shape=17):
    partial_preds = []
...
#     ipdb.set_trace()
    final_preds = np.zeros((data_length,output_shape)).astype(np.float32)
#     data = X_valid[0:16]
...
preds_x10 = make_staged_predictions_v2(resnet_features_model,num_parts=5,nb_aug=10,output_shape=1024)

save_array("data/cache/preds_resnet4a_10xtta_pseudo_fullset_2epochs_featuresonly.dat", preds_x10)
